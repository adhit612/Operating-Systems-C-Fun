...
* shell command to compile: gcc threads.c -o threads -lpthread
* shell command to run: ./threads value(integer)
*/
#include <stdio.h>
#include <stdlib.h>
#include <pthread.h>
...
int main(int argc, char *argv[]) {
    ...
    //need to initalize mutex according ot stackvoverflow
    ...
    /* Implement Code Here */
    if (pthread_create(&t1, NULL, add_counter, "thread 1") != 0) {
        perror("pthread1 create error");
        exit(1);
    }
    if (pthread_create(&t2, NULL, add_counter, "thread 2") != 0) {
        perror("pthread2 create error");
        exit(1);
    }
    if (pthread_create(&t3, NULL, add_counter, "thread 3") != 0) {
        perror("pthread3 create error");
        exit(1);
    }
    if (pthread_create(&t4, NULL, add_counter, "thread 4") != 0) {
        perror("pthread4 create error");
        exit(1);
    }
